...
int GetMessageUDP(HWND hwnd) {
    freopen("server.log", "w", stdout);
    while (!StopRecieve) {
        do {
            sockaddr from{};
            int size = sizeof(from);
            iResult = recvfrom(ListenSocket, recvbuf, recvbuflen, 0, (sockaddr *) &from, &size);
            if (iResult > 0) {
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_CREATE: {
            ...
            iResult = bind(ListenSocket, (const sockaddr *) &addr, sizeof(sockaddr_in));
            ...
            recvthr = std::thread(GetMessageUDP, hwnd);
            return 0;
        }
        case WM_DESTROY:
            // cleanup
